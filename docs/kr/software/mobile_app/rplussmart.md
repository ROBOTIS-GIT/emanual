...
|10150|도형 표시|읽기/쓰기|0|0||4|값 : (도형색상 * 16777216) + (도형크기 * 65536)+ (도형위치 * 256) + (도형번호)<BR/>도형색상 :<BR/>0 : 알 수 없음 1 : 흰색,  2 : 검은색,  3 : 빨간색,  4 : 녹색,  5 : 청색 6 : 노랑색,  7 : 옅은회색, 8 : 회색, 9 : 어두운 회색<BR/>도형크기 : 0 ~ 255 사이의 크기 값<BR/>도형위치 : 1 ~ 25 사이의 화면 영역<BR/>도형번호 :<BR/>0 : 도형 삭제 1 : 원,  2 : 사각,  3 : 삼각<BR/>(읽기) 마지막으로 설정된 도형의 정보<BR/>(쓰기) 화면에 표시할 도형의 정보<BR/>도형위치 = 0, 도형번호 = 0 : 모든 도형 삭제|
...
4. 만약 동일한 번호가 없으면 검색을 통해 등록합니다.(이때, PIN정보가 필요합니다. PIN은 `0000`으로 기본 설정되어 있습니다.)
 ...
 `Note` PIN 정보는 로보티즈 고객지원센터를 통해서만 변경이 가능합니다.
 {: .notice}
