...
- 모션
  ...
  태스크가 로봇이 어떻게 움직일지 논리적인 판단을 정의한 프로그램 코드라면, 모션은 태스크에서 내린 판단에 따라 움직일 행동이 정의된 데이터입니다. 만약, 내가 만든 로봇이 단순한 동작만을 사용하는 로봇이라면 태스크 파일을 작성할 때 모션을 사용하지 않아도 되지만 정밀한 움직임이나 댄스와 같이 복잡하고 다양한 움직임이 필요하다면 모션 파일을 사용하는 것이 효율적입니다 (태스크 코드에서 모션을 사용한다면, 반드시 모션 파일을 다운로드해야 합니다).
...
- 아래는 위치3,3에 숫자를 증가하며 표시하는 예제입니다.
  ...
  ![](/assets/images/sw/rplus_task3/task3_157.png)
...
- 아래는 스마트 기기의 흔들림 정도를 읽어 값이 80이상일 때 화면에 표시한 도형의 색상을 바꾸는 예제입니다.
 ...
 ![](/assets/images/sw/rplus_task3/task3_164.png)
...
- 아래는 스마트 기기의 기울기에 따라 화면에 기울어진 방향에 원을 표시하는 예제입니다.
  ...
  ![](/assets/images/sw/rplus_task3/task3_169.png)
...
- 아래는 조도를 측정하여 주위가 어두우면 회색 원을, 주위가 밝으면 흰색 원을 화면에 표시하는 예제입니다.
  ...
  ![](/assets/images/sw/rplus_task3/task3_172.png)
...
- 아래는 스마트 기기 주위의 자기장을 측정하여 값을 화면에 표시하는 예제입니다.
  ...
  ![](/assets/images/sw/rplus_task3/task3_174.png)
...
- 아래는 소음의 크기에 따라 도형을 표시하는 예제입니다.
  ...
  ![](/assets/images/sw/rplus_task3/task3_179.png)
...
- 아래는 음성 인식과 음성 인식 결과를 사용하여 화면을 터치했을 때 음성을 인식하여 인식된 결과를 화면 중앙에 표시하는 예제입니다.
  ...
  ![](/assets/images/sw/rplus_task3/task3_183.png)
...
- 아래는 터치한 위치에 도형을 표시하는 예제입니다.
  ...
  ![](/assets/images/sw/rplus_task3/task3_186.png)
...
- 아래는 제스처를 인식하여 해당 제스처의 번호를 화면에 출력하는 예제입니다.
  ...
  ![](/assets/images/sw/rplus_task3/task3_188.png)
...
![](/assets/images/sw/rplus_task3/task3_198.png)

![](/assets/images/sw/rplus_task3/task3_199.png)
...
![](/assets/images/sw/rplus_task3/motion_port_021.png)
...
단축키와 함께 키프레임을 더 쉽게 사용할수있습니다.  

Ctrl키를 활용하면 각 프레임 사이의 공간을 사용하여 총 시간에는 변화가 없지만, Insert/Del 키만 활용시에, 각 프레임간의 시간에 변화가 생깁니다.  
...
아래 사진은 Del 키 와 Ctrl 키의 활용의 차이점을 보여줍니다.  
...
**참고** : R+ Task 3.0 두개의 프로그램을 열어서, 모션파일 복사도 가능합니다.
{: .notice--info}
...
2. 로봇을 연결합니다. ([로봇 연결하기](#로봇-연결하기) 참조)  
3. 동기화 모드를 활성화 합니다.  
...
새로운 “로보플러스 Task 3.0 (R+ Task 3.0)”에서는 기존의 “로보플러스 모션1.0 (Roboplus Motion 1.0)” 에서 사용해오던 구 모션 파일(*.mtn)을 그대로 사용할 수 없습니다.   
구 모션 파일(*.mtn)을 새로운 모션 편집기에서 사용하려면 *.mtnx로 변환하는 과정을 거쳐야 합니다.*

**참고** : 기존의 R+Motion 2.0 에서 사용하던 파일은 파일의 확장자(.mtnx)가 동일하기 때문에 그대로 사용가능합니다.  
{: .notice--info}
