...
| 크기          | 24 x 36 x 27 [mm]                                                             |
| 기어비        | 238 : 1                                                                       |
| Stall Torque | 0.39 [Nm] @ 7.4 [V], 1.1 [A]                                                  |
...
현재 전압 값이 이 범위를 벗어날 경우 Status Packet 중 ERROR의 Voltage Range Error Bit(Bit0)가 ‘1’로 설정되어 반환되고, Alarm LED/Shutdown의 플래그(flag)중 입력 전압 에러(Input Voltage Error)가 설정되어 있다면  기능이 발휘됩니다.
