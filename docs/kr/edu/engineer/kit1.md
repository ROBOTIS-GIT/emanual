...
CM-550에 내장된 BLE 슬레이브 모듈의 신호가 불안정(신호의 세기가 약하거나 간섭이 발생)할 경우, 통신이 끊어질 수 있습니다. 로보플러스 매니저 2.0을 사용하여 BLE 신호세기를 알맞게 설정해주세요.

아래 표는 CM-550을 1개 또는 1개 이상의 기기를 한 공간에서 사용 했을 때, 적합한 BLE 신호 세기 설정 가이드입니다.

{% capture ble_setting %}  

| CM-550 1개 | CM-550 1개 이상 | BLE 신호 세기 |
|:----------:|:---------------:|:-------------:|
| 신호 끊김  |    신호 유지    |     증가      |
| 신호 유지  |    신호 끊김    |     감소      |
| 신호 끊김  |    신호 끊김    |     증가      |
...
- 한 공간에서 여러 개가 아닌 하나의 기기를 사용 했을때 통신이 끊긴다면, 신호의 세기가 충분하지 못하다는 의미입니다. **BLE 슬레이브 모듈의 신호세기를 높여주세요**.

- 좁은 공간에서 여러 개의 기기를 사용 했을 때 통신이 끊긴다면, 다른사람의 BLE 신호가 사용자의 BLE 신호에 간섭을 줄 수가 있습니다. **BLE 슬레이브 모듈의 신호세기를 낮춰주세요**. 

- 좁은 공간에서 여러 개의 기기를 사용하고 있음에도 불구하고, 제어기에 내장된 BLE 신호세기가 약하기 때문에 신호가 불안정 할 수 있습니다. **BLE 슬레이브 모듈의 신호세기를 높여주세요**.

- 조건에 관계없이 신호가 잘 유지된다면 신호의 세기가 충분하고, 다른 기기에 의해서 신호간섭이 발생하지 않았다는 의미입니다. **BLE 슬레이브 모듈의 신호세기를 그대로 유지 시켜주세요**.
{% endcapture %}
<div class="notice">{{ ble_setting | markdownify }}</div>

**참고** : 환경의 변화와 사용하는 기기의 수에 따라서, BLE 신호세기 및 간섭의 정도가 달라질수 있습니다.
{: .notice}


### [로보플러스 매니저 2.0으로 BLE 신호 세기 설정하기](#로보플러스-매니저-2.0으로-ble-신호-세기-설정하기)
