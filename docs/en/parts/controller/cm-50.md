...
In order to synchronize right and left wheel speed, you need to tune the motor with a remote controller([RC-100B](#rc-100b), [RC-200](#rc-200), [Virtual Remote Controller](#virtual-remote-controller)).

{% capture notice01 %}  
**NOTE** : Tuning Mode for CM-50 is supported from the Firmware version 77 or above.
- [How to update controller firmware](/docs/en/software/rplus2/manager/)
...
1. Install [R+ m.Task 2.0]{: .blank} on the smart device.
2. Start the CM-50 controller with [Tuning Mode](#tuning-mode).
3. Connect the BT-210 or BT-410 Slave module to CM-50 controller.
