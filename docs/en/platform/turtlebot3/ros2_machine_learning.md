...
To do this tutorial, you need to install Tensorflow and Keras with Ubuntu 18.04 and ROS2 Dashing.

### [ROS dependency packages](#ros-dependency-packages)
You must additionally install ROS dependency packages for Python 3.6 version.
``` bash
...
```
This tutorial is used python 3.6(CPU only). If you want to use another python version and GPU, please refer to [TensorFlow](https://www.tensorflow.org/install/).
``` bash
...
If you want to test your trained model, run below.
``` bash
