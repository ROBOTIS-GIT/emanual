...
**2XL430-W250** is a ground breaking DYNAMIXEL that allows to control **2 axis(2 DOF)** with a single module. In order to control 2 axis at the same time, each axle should be assigned with different ID while sharing an identical Baudrate. Since the Control Table for each axle is separated except the Baudrate, 2XL can be applied in various applications.  
...
**CAUTION** : Each motor on 2XL430 has its own control table. However the value of [Baud Rate(8)] and [Protocol Type(13)], is always changed at the same time because there is only one communication module in the actuator.  
...
**CAUTION** : Please assign two different IDs for the 2XL430. Attempting to assign an identical ID for each joint will result in Data Limit Error(0x06) of the status packet.
{: .notice--warning}

### <a name="baud-rate"></a>**[Baud Rate(8)](#baud-rate8)**
{% include en/dxl/control_table_baudrate_2.md %}

**CAUTION** : Only one Baud Rate can be used for the 2XL430. Attempting to change the Baud Rate of one joint will also affect to the other joint.
{: .notice--warning}
