...
**CAUTION** : Each motor on 2XL430 has its own control table. However the value of **"[Baud Rate(8)](#baud-rate8)"** and **[Protocol Version(13)](#protocol-version13)** is always changed at the same time because there is only one communication module in the actuator.  
In addition, **[Torque Enable(64)](#torque-enable64)** must be set as "0" when changed.
{: .notice--warning}
...
**CAUTION** : Only one Protocol Version can be used for the 2XL430. Attempting to change the Protocol Version of one joint will also affect to the other joint.
{: .notice--warning}
